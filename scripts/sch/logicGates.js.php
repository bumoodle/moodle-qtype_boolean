...
function load_wiring(json_wiring)
{
    console.log(json_wiring);
    ...
    (
        ...
        function(Y)
        {
            ...
            new_val = Y.JSON.parse(json_wiring);
            editor.loadPipe(new_val.working);
        }
    );
}

function wiring_init(m, restore_wiring) 
{ 
    window.editor = new WireIt.WiringEditor(logicGatesLang());     

    if(restore_wiring != "")
        load_wiring(restore_wiring);
